...
class MSVD_DataLoader_original(Dataset):
    """MSVD dataset loader."""
    def __init__(
            self,
            subset,
...
class MSVD_DataLoader(Dataset):
    ...
    def __init__(
        ...
        if self.subset == 'test':
            dict_path = 'data/msvd_test_vitb32_it30_prompt.json'
            self.generate_caps_dict = json.load(open(dict_path, 'r'))
        elif self.subset == 'train':
            dict_path = 'data/msvd_train_vitb32_max1_nopro_final.json'
            self.generate_caps_dict = json.load(open(dict_path, 'r'))
    ...
    def _get_rawframes(self, choice_video_ids):
        ...
        # Pair x L x T x 3 x H x W
        video = np.zeros((len(choice_video_ids), self.max_frames, 1, 3,
                          self.rawFramesExtractor.size, self.rawFramesExtractor.size), dtype=float)
    ...
    def __getitem__(self, idx):
        ...
        if self.subset == 'test':
            ...
            caps_text, caps_mask = self._get_captions(self.generate_caps_dict[video_id]['captions']) ##生成的30句captions
        return pairs_text, pairs_mask, pairs_segment, video, video_mask,caps_text, caps_mask



if __name__ == '__main__':
    data_path =  '/root/paddlejob/workspace/env_run/data/MSVD'
    video_path = '/root/paddlejob/workspace/env_run/data/MSVD/MSVD_videos'
    frame_path = '/root/paddlejob/workspace/env_run/data/MSVD/MSVD_frames'
    import sys  
    sys.path.append('..')
