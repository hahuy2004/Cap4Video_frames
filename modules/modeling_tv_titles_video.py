...
class CLIP4Clip(CLIP4ClipPreTrainedModel):
    def __init__(self, cross_config, clip_state_dict, task_config):
        ...
        # Dòng này dùng để xác định loại tương tác giữa video và text. Mặc định là 'dp' (dot product), nhưng có thể được thay đổi thông qua cấu hình nhiệm vụ (task_config). 
        # Các loại tương tác khác có thể bao gồm 'ti' (token-wise interaction) hoặc 'wti' (weighted token-wise interaction). 
        # Việc lựa chọn loại tương tác sẽ ảnh hưởng đến cách tính toán độ tương đồng giữa video và text, từ đó ảnh hưởng đến hiệu suất của mô hình trong các tác vụ như truy xuất thông tin hoặc phân loại.
        self.interaction = 'dp'
